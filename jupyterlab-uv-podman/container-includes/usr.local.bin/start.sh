#!/usr/bin/bash
# Somewhat modified version of start.sh created by Jupyter Development
# Team. Released under same license. Primary differences are removal of conda
# and change of default username.
...
# shellcheck source=../usr.local.bin/run-hooks.sh
...
# the primary user, and ensure file permissions, grant sudo rights, and such
...
if [ "$(id -u)" == 0 ]; then
    ...
    # Refit the jluv user to the desired user (NB_USER)
    if id jluv &> /dev/null; then
        if ! usermod --home "/home/${NB_USER}" --login "${NB_USER}" jluv 2>&1 | grep "no changes" > /dev/null; then
            _log "Updated the jluv user:"
            _log "- username: jluv       -> ${NB_USER}"
            _log "- home dir: /home/jluv -> /home/${NB_USER}"
        fi
    elif ! id -u "${NB_USER}" &> /dev/null; then
        _log "ERROR: Neither the jluv user nor '${NB_USER}' exists. This could be the result of stopping and starting, the container with a different NB_USER environment variable."
        exit 1
    fi
    ...
    # Move or symlink the jluv home directory to the desired user's home
    # directory if it doesn't already exist, and update the current working
    # directory to the new location if needed.
    if [[ "${NB_USER}" != "jluv" ]]; then
        if [[ ! -e "/home/${NB_USER}" ]]; then
            _log "Attempting to copy /home/jluv to /home/${NB_USER}..."
            mkdir "/home/${NB_USER}"
            # shellcheck disable=SC2086
            if cp ${CP_OPTS:--a} /home/jluv/. "/home/${NB_USER}/"; then
                _log "Success!"
            else
                _log "Failed to copy data from /home/jluv to /home/${NB_USER}!"
                _log "Attempting to symlink /home/jluv to /home/${NB_USER}..."
                if ln -s /home/jluv "/home/${NB_USER}"; then
                    _log "Success creating symlink!"
                else
                    _log "ERROR: Failed copy data from /home/jluv to /home/${NB_USER} or to create symlink!"
                    exit 1
                fi
            fi
        fi
        # Ensure the current working directory is updated to the new path
        if [[ "${PWD}/" == "/home/jluv/"* ]]; then
    ...
    # disabled notebook hooks for now
...
# user we started as.
    ...
    JLUV_UID="$(id -u jluv 2>/dev/null)"  # The default UID for the jluv user
    JLUV_GID="$(id -g jluv 2>/dev/null)"  # The default GID for the jluv user
    ...
    if ! whoami &> /dev/null; then
        ...
        if [[ -w /etc/passwd ]]; then
            _log "Renaming old jluv user to nayvoj ($(id -u jluv):$(id -g jluv))"
            ...
            sed --expression="s/^jluv:/nayvoj:/" /etc/passwd > /tmp/passwd
            echo "${NB_USER}:x:$(id -u):$(id -g):,,,:/home/jluv:/bin/bash" >> /tmp/passwd
            ...
            if [[ "${NB_USER}" != "jluv" ]]; then
                _log "WARNING: user is ${NB_USER} but home is /home/jluv. You must run as root to rename the home directory!"
            fi
        else
    ...
    if [[ ! -w /home/jluv ]]; then
        _log "WARNING: no write access to /home/jluv. Try starting the container with group 'users' (100), e.g. using \"--group-add=users\"."
    fi
